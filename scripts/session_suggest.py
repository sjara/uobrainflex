...
def generate_suggestion(performance):
    ...
    if performance['stage'][1] == '1':
        if all([performance['licks_total']>1000,performance['hits_left_ratio']<.66, performance['hits_left_ratio']>.33]):
            next_session = 'S2'
        else:
            next_session = 'S1'
    elif performance['stage'][1] == '2':
        if all([performance['hits_total']>150,performance['hit_rate_left']>.5, performance['hit_rate_right']>.5]):
            next_session = 'S3'
